module "vpc" {
  source  = "terraform-aws-modules/vpc/aws"
  version = "5.5.3"

  name = "${local.account_name}-vpc"
  cidr = local.vpc_cidr

  azs             = local.azs
  private_subnets = [for k, v in local.azs : cidrsubnet(local.vpc_cidr, 6, k + 10)]
  public_subnets  = [for k, v in local.azs : cidrsubnet(local.vpc_cidr, 6, k)]

  enable_nat_gateway     = true
  create_igw             = true
  enable_dns_hostnames   = true
  single_nat_gateway     = true
  one_nat_gateway_per_az = false
  enable_vpn_gateway     = false

  manage_default_network_acl    = true
  default_network_acl_tags      = { Name = "${local.account_name}-default" }
  manage_default_route_table    = true
  default_route_table_tags      = { Name = "${local.account_name}-default" }
  manage_default_security_group = true
  default_security_group_tags   = { Name = "${local.account_name}-default" }

  public_subnet_tags = {
    "kubernetes.io/role/elb" = 1
  }

  private_subnet_tags = {
    "kubernetes.io/role/internal-elb" = 1
  }

  tags = local.static_tags
}
